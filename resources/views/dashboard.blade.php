...
<head>
    ...
    <style>
        ...
        .neon-border { box-shadow: 0 0 10px rgba(239, 68, 68, 0.3); }
    ...
    <aside class="w-64 bg-[#121520] border-r border-gray-800 flex flex-col justify-between hidden md:flex">
        <div>
            ...
            <div class="h-20 flex items-center px-6 border-b border-gray-800">
                <i class="fa-solid font-orbitron text-2xl text-red-500 mr-3 fa-dice-d20 animate-spin" style="animation-duration: 10s;"></i>
            ...
            <nav class="mt-6 px-4 space-y-2">
                ...
                <a href="#pending-trans" class="flex items-center px-4 py-3 text-gray-400 hover:text-white hover:bg-gray-800/50 rounded-lg font-semibold transition">
                    <i class="fa-solid fa-[#ef4444] fa-clock-rotate-left w-6"></i>
                ...
                <div class="neon-card rounded-2xl p-6 relative overflow-hidden">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <h3 class="text-3xl font-orbitron font-bold text-white mt-2">৳{{ number_format($totalDeposits, 2) }}</h3>
                        </div>
                ...
                <div class="neon-card rounded-2xl p-6 relative overflow-hidden">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <h3 class="text-3xl font-orbitron font-bold text-white mt-2">৳{{ number_format($totalWithdraws, 2) }}</h3>
                        </div>
                ...
                <div class="neon-card rounded-2xl p-6 relative overflow-hidden border-emerald-500/30">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <h3 class="text-3xl font-orbitron font-bold text-emerald-400 mt-2">৳{{ number_format($netProfit, 2) }}</h3>
                        </div>
                            ...
                            <div class="bg-[#121520] p-4 rounded-xl border border-gray-800 flex items-center justify-between">
                                ...
                                <div class="flex gap-2">
                                    <form method="POST" action="{{ route('admin.transaction.status', ['id' => $deposit->id, 'status' => 'approved']) }}">
                                        @csrf
                                        <button class="bg-emerald-600/20 hover:bg-emerald-600 text-emerald-400 hover:text-white border border-emerald-500/40 px-3 py-1.5 rounded-lg text-xs font-bold transition">Approve</button>
                                    </form>
                                    <form method="POST" action="{{ route('admin.transaction.status', ['id' => $deposit->id, 'status' => 'rejected']) }}">
                                        @csrf
                                        <button class="bg-red-600/20 hover:bg-red-600 text-red-400 hover:text-white border border-red-500/40 px-3 py-1.5 rounded-lg text-xs font-bold transition">Reject</button>
                                    </form>
                                </div>
                            </div>
                        @empty
                ...
                <div class="neon-card rounded-2xl p-6">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <span class="bg-purple-500/20 text-purple-400 text-xs px-3 py-1 rounded-full border border-purple-500/30 font-bold">
                            {{ $pendingWithdraws->count() }} Waiting
                        </span>
                    </div>
                        ...
                        <tbody class="divide-y divide-gray-800/60 text-gray-300">
                            @forelse($transactions as $trx)
                                <tr class="hover:bg-gray-800/30">
                                    ...
                                    <td class="py-3 px-4 text-xs text-gray-500">{{ $trx->created_at->format('M d, Y h:i A') }}</td>
                                </tr>
                            @empty
                                <tr>
